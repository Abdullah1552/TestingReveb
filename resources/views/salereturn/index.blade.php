                        ...
                        <form id="form">
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label>SR#</label>
                                    <input name="sr" class="form-control" placeholder="Enter Sr#">
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
    ...
    <script>
        ...
        function appendData(qty, htmlElement)
        {
            ...
            $(document).on('keyup change', '#order_shipping_input', function() {
                ...
                let orderDiscount = parseInt($('#order_discount_input').val());
                if(isNaN(orderDiscount)){
                    orderDiscountDecimal=0;
                }else{
                    let orderDiscountDecimal = orderDiscount.toFixed(2);
        ...
        function get_data(page){
            ...
            $.ajax({
                url:'{{url('returns/get_sale_return')}}',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"POST",
                data:$("#form").serialize(),
                dataType:"JSON",
                success:function (data) {
                    ...
                    for(i in data.data){
                        ...
                        htmlData+=' <button type="button" onclick="del_rec(\''+data.data[i].id+'\', \'{{ url('/returns/sale_return/') }}/'+data.data[i].id+'\')" class="btn btn-mini btn-danger"><i class="fa fa-trash-o"></i> </button>';
                        @endcan
                        htmlData+=' <a href="{{ url('returns/sale_return_print') }}/'+data.data[i].id+'" target="_blank"  class="btn btn-mini btn-info"><i class="fa fa-print"></i> </a>';
                        htmlData+='</td>';
                        htmlData+='</tr>';
        ...
        function edit(id) {
            ...
            $("#purchaseorder").modal();
            $.ajax({
                url:"{{ url('returns/sale_return') }}/"+id+"/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success:function (data) {
                    ...
                    $("#purchase-form select[name~='customer_id']").val(data[0].customer_id);
                    $('.loader-bg').hide();
                    $("#total").text((parseFloat(data[0].net_total)-parseFloat(data[0].shipping_cost)+parseFloat(data[0].discount)).toFixed(2));
                    $("#order_discount").text(data[0].discount);
                    $("#shipping_cost").text(data[0].shipping_cost);
                    $("#net-total").val(parseFloat(data[0].net_total));
                    $("#grand_total").text(parseFloat(data[0].net_total));

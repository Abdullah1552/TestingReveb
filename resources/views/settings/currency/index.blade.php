...
@section('content')
    ...
    @include('settings.currency.modal');
    <script>
        function add_new() {
            $("#new-sub_head").modal();
            $("#currency-form input[name~='id']").val(0);
            ...
            $.ajax({
                ...
                success:function (data) {
                    ...
                    $.each(vali, function( index, value ) {
                        $("#currency-form input[name~='" + index + "']").css('border', '1px solid red');
                        toastr.error(''+value+'');
                    });
                    $('.loader-bg').hide();
                }
        ...
        function get_data(page){
            $.ajax({
                url:"{{ url('settings/get_currency') }}?page="+page,
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"POST",
                dataType:"JSON",
                success:function (data) {
                    ...
                    for(i in data.data){
                        ...
                        htmlData+='<td>'+data.data[i].exchange_rate+'</td>';
                        htmlData+='<td>';
                        @can('currency_edit')
                        htmlData+='<a class="btn btn-mini btn-primary" href="javascript:void(0)" onclick="edit('+data.data[i].id+')"><i class="fa fa-edit"></i></a>';
                        @endcan
                            @can('currency_delete')
                        htmlData+=' <button type="button" onclick="del_rec(\''+data.data[i].id+'\', \'{{ url('/settings/currency/') }}/'+data.data[i].id+'\')" class="btn btn-mini btn-danger"><i class="fa fa-trash-o"></i> </button>';
                        @endcan
                        htmlData+='</td>';
                        htmlData+='</tr>';
                    }
        ...
        function edit(id) {
            ...
            $("#new-sub_head").modal();
            $.ajax({
                url:"{{ url('settings/currency') }}/"+id+"/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success:function (data) {
                    $("#currency-form input[name~='id']").val(data.id);
                    $("#currency-form input[name~='name']").val(data.name);
                    $("#currency-form input[name~='code']").val(data.code);
                    $("#currency-form input[name~='exchange_rate']").val(data.exchange_rate);
                    $('.loader-bg').hide();
                }
            })
        }

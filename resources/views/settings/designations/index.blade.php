...
@section('content')
    ...
    @include('settings.designations.modal');
    <script>
        function add_new() {
            $("#new-sub_head").modal();
        ...
        function get_data(page){
            $.ajax({
                url:"{{ url('get_designations') }}?page="+page,
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"POST",
                dataType:"JSON",
                success:function (data) {
                    ...
                    for(i in data.data){
                        ...
                        htmlData+='<td>'+data.data[i].Designation+'</td>';
                        htmlData+='<td>';
                        htmlData+='<a class="btn btn-mini btn-primary" href="javascript:void(0)" onclick="edit('+data.data[i].id+')"><i class="fa fa-edit"></i></a>';
                        htmlData+=' <button type="button" onclick="del_rec(\''+data.data[i].id+'\', \'{{ url('/designations/') }}/'+data.data[i].id+'\')" class="btn btn-mini btn-danger"><i class="fa fa-trash-o"></i> </button>';
                        htmlData+='</td>';
                        htmlData+='</tr>';
                    }
        ...
        function edit(id) {
            ...
            $("#new-sub_head").modal();
            $.ajax({
                url:"{{ url('designations') }}/"+id+"/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success:function (data) {
                    $("#cost_center-form input[name~='id']").val(data.id);
                    $("#cost_center-form input[name~='Designation']").val(data.Designation);
                    $('.loader-bg').hide();
                }
            })
        }

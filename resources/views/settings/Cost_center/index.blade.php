...
@section('content')
            ...
            <div class="row">
                <div class="main-header" style="margin-top: 0px;">
                    ...
                    <ol class="breadcrumb breadcrumb-title breadcrumb-arrow">
                        ...
                        <li class="breadcrumb-item">Cost Center List </li>
                    </ol>
                </div>
            </div>
            <!-- End start -->
    ...
    @include('settings.Cost_center.modal');
    <script>
        function add_new() {
            $("#new-sub_head").modal();
        ...
        function get_data(page){
            $.ajax({
                url:"{{ url('get_cost_center') }}?page="+page,
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"POST",
                dataType:"JSON",
                success:function (data) {
        ...
        function edit(id) {
            ...
            $("#new-sub_head").modal();
            $.ajax({
                url:"{{ url('cost_center') }}/"+id+"/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success:function (data) {
                    $("#cost_center-form input[name~='id']").val(data[0].id);
                    $("#cost_center-form input[name~='CC_Name']").val(data[0].CC_Name);
                    $('.loader-bg').hide();
                }
            })
        }

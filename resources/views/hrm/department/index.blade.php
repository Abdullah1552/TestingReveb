...
@section('content')
    ...
    <script>
        ...
        function get_data(page){
            $.ajax({
                url:"{{ url('/hrm/get_departments') }}?page="+page,
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"POST",
                dataType:"JSON",
                success:function (data) {
                    ...
                    for(i in data.data){
                        ...
                        htmlData+=' <button type="button" onclick="del_rec(\''+data.data[i].id+'\', \'{{ url('/hrm/department/') }}/'+data.data[i].id+'\')" class="btn btn-mini btn-danger"><i class="fa fa-trash-o"></i> </button>';
                        @endcan
                        htmlData+='</td>';
                        htmlData+='</tr>';
                    }
        ...
        function edit(id) {
            ...
            $("#new-sub_head").modal();
            $.ajax({
                url:"{{ url('/hrm/department') }}/"+id+"/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success:function (data) {
                    $("#cost_center-form input[name~='id']").val(data.id);
                    $("#cost_center-form input[name~='department_name']").val(data.department_name);
                    $('.loader-bg').hide();
                }
            })
        }

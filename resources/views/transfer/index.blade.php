...
@section('content')
    <style type="text/css">
        ...
        @media print {
            ...
            .page-title, .main-header .panel-heading, hr{
                display: none;}
    ...
    <div class="content-wrapper">
        <div class="container-fluid">
            ...
            <!-- End start -->
                ...
                <div class="col-md-12">
                    <div class="card">
                        <form id="filter-search" class="no-report">

                            <div class="col-md-2">
                                <div class="form-group">
                                    <label>Date From</label>
                                    <input name="df" class="form-control date" value="">
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label>Date To</label>
                                    <input name="dt" class="form-control date" value="">
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label>From Location *</label>
                                    <select class="js-example-basic-single form-control" name="WHIDF">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <select class="js-example-basic-single form-control" name="WHIDT">
                                        <option value="">Select Location</option>
                        ...
                        <form id="transfer-data">
                            <div class="card-block">
                                ...
                                <div class="col-sm-12 table-responsive pad0">
                                    ...
                                    <h4 class="print-header" align="center" style="display:none;margin-bottom: 0px;margin-top: 5px;font-size: 14px;">Transfer Report</h4>

                                    <table class="table table2excel">

                                        <tr style="background-color: #eeeeee">
                                            <th  class="no-report" scope="col"><input type="checkbox" id="select_all" /></th>
                                            <th>#</th>
                                            <th>Date</th>
                                            <th>Description</th>
                                            <th>Location(From)</th>
                                        ...
                                        <tr>
                                            <td colspan="9" class="no-report" align="right"> <button type="button" onclick="del_multiple_rec('transfer-data','/transfers/delete_multiple_transfers')" class="btn btn-mini btn-danger"><i class="fa fa-trash-o"></i> selected</button></td>
                                        </tr>
                                    </table>
    ...
    <script>
        ...
        $('#tranfer-form').submit(function(e) {
            e.preventDefault();
            $('.loader-bg').show();
            ...
            $.ajax({
                ...
                data:formData,
                success:function (data) {
                    $("#tranfer-form input[name~='id']").val(0);
                    toastr.success('Operation successfull.');
                    document.getElementById("tranfer-form").reset();
                    $('.rows').remove()
                    $('#item-count').text('0(0)')
                    $('#subtotal').text(0)
                    $('#grand_total').text(0);
        ...
        function get_data(page){
            ...
            let Total_qty=0;
            $.ajax({
                url:'{{url('transfers/get_transfers')}}',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type:"POST",
                dataType:"JSON",
                ...
                success:function (data) {
                    ...
                    htmlData='';
                    for(i in data.data){
                        let date=data.data[i].date.substring(0, 10);;
                        ...
                        htmlData+='<td>'+date+'</td>';
                        htmlData+='<td>Transfer - '+data.data[i].id+'</td>';
                        htmlData+='<td>'+data.data[i].location_from.WH_Name+'</td>';
                        htmlData+='<td>'+data.data[i].location_to.WH_Name+'</td>';
                        htmlData+='<td>'+data.data[i].total_QTY+'</td>';
                        htmlData+='<td>'+transfer_status(data.data[i].status)+'</td>';
                        htmlData+='<td  class="no-report">';
                        @can('transfer_list_edit')
                        htmlData+='<a class="btn btn-mini btn-primary" href="javascript:void(0)" onclick="edit('+data.data[i].id+')"><i class="fa fa-edit"></i></a>';
                        @endcan
                            @can('transfer_list_delete')
                        htmlData+=' <button type="button" onclick="del_rec(\''+data.data[i].id+'\', \'{{ url('/transfers/transfer') }}/'+data.data[i].id+'\')" class="btn btn-mini btn-danger"><i class="fa fa-trash-o"></i> </button>';
                        @endcan
                        htmlData+=' <a class="btn btn-mini btn-default" target="_blank" href="{{ url('transfers/print_transfers') }}/'+data.data[i].id+'"><i class="fa fa-print"></i></a>';
                        htmlData+='</td>';
                        htmlData+='</tr>';
        ...
        function edit(id) {
            ...
            $("#new").modal();
            $.ajax({
                url:"{{ url('transfers/transfer') }}/"+id+"/edit",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success:function (data) {
                    $('#myTable tr:last').before(data.product_line);
                    $("#tranfer-form input[name~='id']").val(data.result.id);
                    $("#tranfer-form select[name~='WHIDF']").val(data.result.WHIDF);
                    $("#tranfer-form select[name~='WHIDT']").val(data.result.WHIDT);
                    $("#tranfer-form select[name~='status']").val(data.result.status);
                    let qty=0;
                    let sub_total=0;
                    $(".quantity").each(function () {
                        qty+=Number($(this).val());
                    ...
                    // $('#net-total').val((data.result.net_total));
                    // $('#grand_total').text((data.result.net_total));
